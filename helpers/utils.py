...
# Load label (relative to this file so it works when installed as a package)
_csv_path = Path(__file__).parent.parent / "metadata" / "class_labels_indices.csv"
labels: list = []
ids: list = []
classes_num: int = 527  # default AudioSet
if _csv_path.exists():
    with open(_csv_path) as f:
        reader = csv.reader(f, delimiter=',')
        lines = list(reader)
    for i1 in range(1, len(lines)):
        ids.append(lines[i1][1])
