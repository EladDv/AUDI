...
def feat_scalogram(y, cfg):
    """Constant-Q transform (log-frequency, great for low-freq rotor tones).
    Bins are sized to stay under Nyquist; the result is resized to (F, T)."""
    import librosa
    fmin = max(cfg.fmin, 32.0)
    fmax = min(cfg.fmax, 0.95 * cfg.sr / 2.0)         # keep top bin below Nyquist
    bpo = 36
    n_bins = int(np.floor(bpo * np.log2(fmax / fmin)))
    n_bins = max(n_bins, 24)
    n_oct = int(np.ceil(n_bins / bpo))
    step = 1 << n_oct                                  # cqt hop must be a multiple
...
def feat_mel3(y, cfg):
    """[PCEN-mel, scalogram, modulation] - the 3-channel feature used by mel3_cnn.

    ch0 PCEN-mel  -> detection energy
    ch1 scalogram -> >4 kHz type separator (EVO low / FPV high)
    ch2 modulation-> rotor rhythm/RPM
    """
...
def type_physics_stats(clip, cfg):
    """The 9 EVO/FPV physics cues for one clip. Same as training/features.py.

    Used both for EVO-vs-FPV typing AND (in detect_server) as a drone-ness gate:
    speech/music land many sigma from BOTH drone references, real drones land close.
    """
