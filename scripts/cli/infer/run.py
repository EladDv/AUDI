...
def run() -> None:
    ...
    CKPT = (
        ROOT
        / "checkpoints_v2/phase3_full/scratch_resnet18/lightning_logs/version_0/checkpoints/epoch=14-step=1500.ckpt"
    )  # noqa: E501
    ...
    print(
        f"  {len(bg_p)} bg chunks: min={bg_p.min():.4f} max={bg_p.max():.4f} mean={bg_p.mean():.4f}"
    )
    thresh_fpr10 = np.percentile(bg_p, 90)
    print(
        f"  FPR=10% threshold: {thresh_fpr10:.4f}  (actual FPR={(bg_p >= thresh_fpr10).mean():.3%})"
    )
    ...
    print(
        f"  {len(d_p)} chunks: min={d_p.min():.4f} max={d_p.max():.4f} mean={d_p.mean():.4f}"
    )
    w = 4
    ...
    nb = len(drone_full) // bn
    sil = np.abs(drone_full[: nb * bn]) < 1e-5
    blk_s = np.array(
        [sil[i * bn : (i + 1) * bn].mean() > 0.7 for i in range(nb)]
    )
    runs3 = []
    in_r = False
    r_start = 0.0
    for i, s in enumerate(blk_s):
        ...
        if not s and not in_r:
            ...
            dur = t - r_start
            if dur >= 0.5:
                runs3.append((r_start, t, dur))
            in_r = False
    if in_r:
        dur = nb * bs - r_start
        if dur >= 0.5:
            runs3.append((r_start, nb * bs, dur))
    merged = []
    for r in runs3:
    ...
    for j, (rs_s, re_s, dur) in enumerate(runs):
        ...
        if len(rt) < 2:
            continue
        dj = 200 * (1 - (rt - rs_s) / dur)
        si = np.argsort(dj)
        ad[j] = np.interp(dg, dj[si], rp[si])
    md = np.nanmean(ad, axis=0)
    sd = np.nanstd(ad, axis=0)
        ...
        json.dump(
            dict(
                config={
                    ...
                    "chunk_s": CHUNK_S,
                    "overlap": OVERLAP,
                    "thresh_fpr10": thresh_fpr10,
                },
                drone_times=d_t.tolist(),
                drone_probs=d_p.tolist(),
                running_avg=ra.tolist(),
                runs=[
                    {
                        "id": i + 1,
                        "start": float(s),
    ...
    gs = fig.add_gridspec(
        4, 2, hspace=0.45, wspace=0.3, height_ratios=[2.5, 2.3, 2.3, 2.5]
    )
    ...
    ax.set_title(
        "Mel Spectrogram (RMS-norm'd) + Runs", fontsize=12, fontweight="bold"
    )
    ...
    ax.axhline(
        ...
        ls="--",
        lw=1,
        label=f"FPR=10% ({thresh_fpr10:.3f})",
    )
    for j, (s, e, _d) in enumerate(runs):
        ax.axvspan(s, e, alpha=0.08, color=colors[j])
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Detection Prob")
    ax.legend(fontsize=7)
    ax.set_ylim(-0.02, 1.02)
    ax.grid(alpha=0.25)
    ax.set_title("Detection vs Time")
    ...
    for j, (s, e, d) in enumerate(runs):
        ...
        if len(rt) < 2:
            continue
        ax.plot(
            200 * (1 - (rt - s) / d), dp, lw=0.5, alpha=0.35, color=colors[j]
        )
    ...
    ax.grid(alpha=0.25)
    ax.legend(fontsize=7)
    ax.set_title("Detection at Distance Markers")
    ax = fig.add_subplot(gs[2, 1])
    xp = np.arange(len(runs))
    wd = 0.35
    hr, mp = [], []
    for s, e, d in runs:
        m = (d_t >= s) & (d_t <= e)
    ...
    ax.hist(
        bg_p, bins=60, color="gray", alpha=0.6, density=True, label="Background"
    )
    ax.hist(
        d_p,
        bins=60,
    ...
    ax.set_xlabel("Detection Prob")
    ax.set_ylabel("Density")
    ax.legend(fontsize=8)
    ax.grid(alpha=0.2, axis="y")
    ax.set_title("Prob Distribution: BG vs Drone")
    ...
    fig.suptitle(
        "audi CNN14: Detection vs Distance (RMS-normalized, 200m→0m)",
        fontsize=15,
        fontweight="bold",
        y=0.99,
    )
