...
class HearabilityEstimator:
    """Bayesian SNR-bin estimator for detection logits."""
    ...
    def from_predictions(cls, pred_path: Path) -> "HearabilityEstimator":
        ...
        unique_bins = sorted(set(bin_name for bin_name in pos_bins if bin_name))
        if not unique_bins:
    ...
    def predict(self, logit: float) -> dict[str, float]:
        ...
        for bin_stats in self.bins:
            std = max(bin_stats.std, 1e-8)
            z = (logit - bin_stats.mean) / std
            log_likelihood = -0.5 * z * z - np.log(std)
            log_probs[bin_stats.name] = log_likelihood + np.log(
                max(bin_stats.prior, 1e-12)
            )
    ...
    def load(cls, path: Path) -> "HearabilityEstimator":
        data = np.load(path)
        bins = [
            BinStats(str(name), float(mean), float(std), float(prior))
            for name, mean, std, prior in zip(
